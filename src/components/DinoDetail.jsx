...
function DetailCanvasBg() {
  ...
  React.useEffect(() => {
    ...
    const particles = Array.from({ length: 30 }, () => ({
      ...
      r: Math.random()*1.2+0.3, alpha: Math.random()*0.3+0.1,
      color: ['123,147,248','96,165,250','45,212,160'][Math.floor(Math.random()*3)],
    }));
    const draw = () => {
      ...
      for(let i=0;i<particles.length;i++) for(let j=i+1;j<particles.length;j++){
        ...
        ctx.strokeStyle=`rgba(123,147,248,${0.04*(1-d/100)})`;ctx.lineWidth=0.5;ctx.stroke();}
      }
